...
Côté Back, nous avons particulièrement ressenti ces difficultés de manipulation du PHP : j'ai notamment rencontré de nombreux "bugs", des syntaxes un peu particulières à respecter, le cache PHP à vider... Ces erreurs n'étaient pas faciles à débuguer car elles ne sont pas forcément apparentes.

A refaire, il y a plusieurs points que j'aurais fait différemment :

* le choix du langage : j'aurais préféré réaliser un tel projet en Java, en utilisant le framework Spring ou SpringBoot. En effet, il s'agit d'un langage que je maîtrise davantage et qui, d'un point de vue personnel, me sert plus (et que j'utilise en entreprise notamment). Je pense en effet que le langage que nous avons choisi nous a fait perdre un peu de temps au début : les bugs rencontrées, le fait que nous étions tous débutants dans ce langage a fait que nous avons mis du temps à nous lancer complètement dans la réalisation du site.
* l'organisation du travail et du temps : le temps qu'on a mis (plusieurs séances) à mettre en place un répertoire Git fonctionnel et le nombre de personnes à travailler sur le BackEnd et à réaliser les tests, insuffisant.
...
J'ai beaucoup apprécié le fait que l'on puisse choisir un sujet libre pour notre projet de Génie Logiciel. En effet, ce projet a fut l'occasion de répondre à un besoin réel d'un client souhaitant faire réaliser un site vitrine, avec une partie blog et une partie dédiée à la présentation et à la mise en vente de ses produits, principalement du miel. La nécéssité de pouvoir héberger ce projet web ainsi que les consignes du projets nous ont dirigé vers le choix du framework symfony avec lequel je n'avais jamais travaillé. J'ai donc eu l'occasion d'apprendre à maitriser ce framework pour réaliser principalement des développements coté back-end en php avec la syntaxe propre à symfony. J'ai également découvert le langage twig lors de la réalisation des templates.
Pour travailler en équipe nous avons appris à nous servir de l'outil git en association avec un dépôt GitHub.
Une des améliorations possible de notre projet serait l'ajout d'une option d'achat en ligne des différents produits présentés sur le site. Nous voulions implémenter cette fonctionnalitée avec l'API que propose PayPal mais nous avons manquer de temps.
Concernant le cour, j'ai apprécié la diversité de son contenu et le fait qu'il soit hébergé et donc que l'on puisse y accéder simplement en tapant l'url associé.