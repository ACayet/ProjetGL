...
J'ai beaucoup apprécié le fait que l'on puisse choisir un sujet libre pour notre projet de Génie Logiciel. En effet, ce projet a fut l'occasion de répondre à un besoin réel d'un client souhaitant faire réaliser un site vitrine, avec une partie blog et une partie dédiée à la présentation et à la mise en vente de ses produits, principalement du miel. La nécessité de pouvoir héberger ce projet web ainsi que les consignes du projet nous ont dirigé vers le choix du framework symfony avec lequel je n'avais jamais travaillé. J'ai donc eu l'occasion d'apprendre à maitriser ce framework pour réaliser principalement des développements côté back-end en php avec la syntaxe propre à symfony. J'ai également découvert le langage twig lors de la réalisation des templates.
Pour travailler en équipe nous avons appris à nous servir de l'outil git en association avec un dépôt GitHub.
Une des améliorations possibles de notre projet serait l'ajout d'une option d'achat en ligne des différents produits présentés sur le site. Nous voulions implémenter cette fonctionnalité avec l'API que propose PayPal ,mais nous avons manqué de temps.
Concernant le cours, j'ai apprécié la diversité de son contenu et le fait qu'il soit hébergé et donc que l'on puisse y accéder simplement en tapant l'url associé.